...
if [[ $SIM == 'true' ]]; then ROBOT_NAME_YAML_FILE=`rospack find iliad_launch_system`/config/robots/robot2_sim.yaml; else ROBOT_NAME_YAML_FILE=`rospack find iliad_launch_system`/config/robots/robot2.yaml; fi
